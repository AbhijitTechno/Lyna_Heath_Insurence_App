...
class CustomAuthManager {
  ...
  Future signOut() async {
    ...
    lynaAuthUserSubject.add(
      LynaAuthUser(loggedIn: false),
    );
    persistAuthData();
  }

  Future<LynaAuthUser?> signIn({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
  ...
  LynaAuthUser? _updateCurrentUser({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
    ...
    final updatedUser = LynaAuthUser(
      loggedIn: true,
      uid: authUid,
      userData: userData,
    );
    lynaAuthUserSubject.add(updatedUser);
    persistAuthData();
    return updatedUser;
  }
  ...
  Future initialize() async {
    ...
    try {
      ...
      userData = _prefs.getString(_kUserDataKey) != null
          ? UserDetailsStruct.fromSerializableMap(
